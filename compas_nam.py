...
# n_interactions_list = [0, 18, 'full']
n_interactions_list = [0]
num_learners = 100
    ...
    for row_idx, scale_pattern in enumerate([False, True]):
        for feature_idx, feature_name in enumerate(feature_names):
            ...
            if is_categorical:
                ...
                for preds in preds_all:
                    ax.plot(x_vals_real, preds, color='blue', alpha=0.1, linewidth=1)
            ...
            if row_idx == 0:
                ...
                print(global_min_raw, global_max_raw)
            else:
                ax.set_ylim(global_min_pattern, global_max_pattern)
                if row_idx == 1:
                    print(global_min_pattern, global_max_pattern)


            if row_idx == 0 and feature_idx == 0:
                ax.set_ylabel('Recidivism Risk', fontsize=14)
            if row_idx == 1 and feature_idx == 0:
                ax.set_ylabel('PatternGAM\nRecidivism Risk', fontsize=16)

            if row_idx == 0:
                ax.set_title(feature_map[feature_name], fontsize=16)

            ax.tick_params(labelsize=10)

    plt.tight_layout()
    plt.savefig(f"{OUT_DIR}/recid_combined_{n_interactions}ints_{num_learners}models.png")
    plt.savefig(f"{OUT_DIR}/recid_combined_{n_interactions}ints_{num_learners}models_hires.png", dpi=300)
    plt.close()
